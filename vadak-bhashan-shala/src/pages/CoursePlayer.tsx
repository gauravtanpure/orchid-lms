...
// --- UPGRADED API FETCH FUNCTION (WITH DEBUGGING) ---
const fetchCourseBySlug = async (courseSlug: string, token: string | null): Promise<CourseDetails> => {
  ...
  if (!courseSlug || courseSlug === 'undefined') {
    console.error('[DEBUG fetchCourseBySlug] invalid courseSlug:', courseSlug);
    throw new Error('Course slug is invalid or missing.');
  }

  // Build URL for protected player route
  const url = `${API_URL}/api/courses/player/${courseSlug}`;
  console.log('[DEBUG fetchCourseBySlug] Preparing request', { url, tokenPresent: !!token });

  // If token is missing, we still attempt request but log clearly
  if (!token) {
    console.warn('[DEBUG fetchCourseBySlug] No token provided — request will likely be rejected by the backend (401).');
  }

  try {
    // Explicitly include Authorization header to avoid relying on interceptors while debugging
    const requestConfig = {
      headers: {
        Authorization: token ? `Bearer ${token}` : undefined,
        // avoid caching to see fresh responses during debug
        'Cache-Control': 'no-cache',
        Pragma: 'no-cache',
      }
    };

    console.log('[DEBUG fetchCourseBySlug] Sending GET request to backend (Network tab should show this).');
    const response = await axios.get(url, requestConfig);

    // Detailed response logging (avoid huge dumps in production)
    console.log('[DEBUG fetchCourseBySlug] Response received:', {
      status: response.status,
      statusText: response.statusText,
      dataPresent: !!response.data,
      lessonsCount: Array.isArray(response.data?.lessons) ? response.data.lessons.length : 'N/A',
    });

    // Sanity check: ensure response.data exists
    if (response.status === 200 && !response.data) {
      console.error('❌ [DEBUG] Server returned 200 OK but response.data is empty!');
      throw new Error('Server returned empty course data.');
    }

    return response.data;
  } catch (err: any) {
    // Log as much useful info as possible for debugging
    console.error('[DEBUG fetchCourseBySlug] Request failed:', {
      message: err.message,
    ...
    // Re-throw a helpful message for react-query UI
    throw new Error(err.response?.data?.message || err.message || 'Unknown server error');
  }
};
...
const CoursePlayer: React.FC = () => {
  ...
  // Simple mount/debug logs
  console.log('[DEBUG] CoursePlayer mounted');
  console.log('[DEBUG] useParams slug =>', slug);
  console.log('[DEBUG] auth state =>', { isAuthenticated, tokenPresent: !!token, isAuthLoading });
  ...
  const {
    ...
    // Temporarily enabled true for debugging; change back to your preferred condition later
    enabled: true,
    retry: 1,
    onError: (err) => console.error('[DEBUG React Query Error] ', err),
  });

  // Log when course loads/changes
  useEffect(() => {
    if (course) {
      console.log('[DEBUG] course data changed:', {
        id: course._id,
        title: course.title,
  ...
  useEffect(() => {
    if (course && Array.isArray(course.lessons) && course.lessons.length > 0 && !currentLesson) {
      const sortedLessons = [...course.lessons].sort((a, b) => a.order - b.order);
      setCurrentLesson(sortedLessons[0]);
      console.log('[DEBUG] default currentLesson set =>', sortedLessons[0]._id);
    }
  }, [course, currentLesson]);

  // Additional log when currentLesson changes
  useEffect(() => {
    if (currentLesson) {
      console.log('[DEBUG] currentLesson changed =>', { id: currentLesson._id, title: currentLesson.title });
    }
  }, [currentLesson]);
  ...
  if (isError || !course) {
    ...
    console.log('[DEBUG] Rendering error UI', { isError, coursePresent: !!course, errorMessage });
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-grow container mx-auto px-4 py-16 text-center">
          ...
          <div className="mt-4 space-x-2">
            <Button onClick={() => { console.log('[DEBUG] manual refetch triggered'); refetch(); }}>Retry</Button>
            <Link to="/courses">
